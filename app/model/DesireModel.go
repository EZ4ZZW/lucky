...
func (model *Desire) LightDesire(data Desire) helper.ReturnType {
	...
	if err != nil {
		return helper.ReturnType{Status: common.CodeError, Msg: "点亮愿望失败", Data: err.Error()}
	}
	...
	if desire.State == 0 {
		...
		if err != nil {
			return helper.ReturnType{Status: common.CodeError, Msg: "点亮愿望失败,数据库错误", Data: err.Error()}
		}
...
func (model *Desire) CancelAchieveDesire(data Desire) helper.ReturnType {
	err := db.
		...
		Error
	if err != nil {
		return helper.ReturnType{Status: common.CodeError, Msg: "取消实现愿望失败", Data: err.Error()}
	}

	return helper.ReturnType{Status: common.CodeSuccess, Msg: "取消实现愿望成功", Data: nil}
